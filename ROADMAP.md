...
| LEG-18 | `test_known_yaml.c` compiled a copy of the wifi_daemon parser that had already diverged on the missing-file return the seed fallback branches on. One implementation now, `sdk/net/known_yaml.c`, compiled by the host suite. **On-board `iw join` / reboot / `iw status` is still owed** — a board with a hand-edited `known.yaml` parses differently after this | major | S | — | [specs/SPEC-leg-17-18-19-21-tests-that-cover.md](specs/SPEC-leg-17-18-19-21-tests-that-cover.md) | **CODE COMPLETE**, on-board validation open |
