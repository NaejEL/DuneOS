...
**`docs/adr/` holds 42 ADRs today** (`docs/adr/*.md` minus `README.md`), not 13: ADR 013 to ADR 041 were written after this phase, by the phases and sprints that needed them.
...
- [x] **25.5 — Standard config + image format** (shipped 2026-05-22)
  ...
  - `splash` migration: if `/etc/splash/config.yaml` contains `logo: /etc/splash/logo.dr`, blit the logo centred (libimage), otherwise procedural dune art. The CardPuter tracks `boards/m5stack-cardputer/etc/splash/logo.png`; `dbt flashimg` converts it to `logo.dr` at stage time (aspect-preserving, downscaled to the panel). No `.dr` is tracked, and the root `duneos_logo.png` it was once said to derive from is gone (LEG-13).
...
| LEG-05 | `duneos_loader_load()` 272 lines (not the 229 the audit recorded — it had grown), 10 steps, 6 resources on a single `out` label | minor | M | LEG-25 | [specs/SPEC-leg-05-split-loader-load.md](specs/SPEC-leg-05-split-loader-load.md) | **DONE** — 76 lines, a flat sequence of named `static` steps with all state in a `load_state_t` and one `load_release()`. Behaviour identity verified at the object level (all string literals identical, 85 klog messages unchanged, call-site counts equal, every rc mapping preserved). **Designed against frame size, not readability**, because this is the LEG-37 path: `duneos_loader_load` keeps its 160 B frame and the four deepest-framed callees (`duneos_elf_image_open` 96 B, `extract_manifest` 64 B, `load_sections` 80 B, `apply_relocations` 112 B) are still called directly from it — nothing was pushed behind a step helper. Measured on hardware across three boots, which report **free** space: 924/860/844 B free. `main_task`'s real stack is 5120 B, not the board's 4608 B Kconfig value (`ESP_TASK_MAIN_STACK` adds the 512 B `TASK_EXTRA_STACK_SIZE` of this picolibc build), so the peaks are 4196/4260/4276 B of 5120 B. The worst boot leaves 844 B raw, **784 B after the 60 B watchpoint — the same 784 B it started from**: the worst-case margin is unchanged, not improved. `main_task` also gained its first instrumentation — total and peak published to `/tmp/state/stack` (ADR 027) and shown by `free`, deliberately not a `duneos_meminfo_t` field, which would write past an already-compiled app's buffer. The header stack ledger carried a false claim in each of its first four versions; correcting the fourth is what uncovered **LEG-41** |
...
**One detail of LEG-36 generalised past it and was fixed separately.** `flashimg.py` hardcoded `_SYSBIN_SIZE = 0x100000` — an unlinked duplicate of the partition table. It now reads the board's `sysbin` row through `system.parse_partition_sizes()` and refuses by name when there is none. `duneos-bspgen.py` keeps its own constant: it is the producer.
...
| ID | Finding | Severity | Size | Depends on | Spec | Status |
|---|---|---|---|---|---|---|
| LEG-06 | No LICENSE file on a public repository open to PRs | major | XS | — | [specs/SPEC-leg-pin-and-share-dependencies.md](specs/SPEC-leg-pin-and-share-dependencies.md) | DONE |
| LEG-07 | `.gitignore:15` (`*.lock`) hid `dependencies.lock`, which commit `6328583` had already dropped from the index — the audit's "tracked today" reading was wrong. Rule removed, lock re-tracked | major | XS | — | [specs/SPEC-leg-pin-and-share-dependencies.md](specs/SPEC-leg-pin-and-share-dependencies.md) | DONE |
...
| LEG-09 | `_DEPS` (`tools/dbt.py:25`) and the four `pip install` lines in CI without version constraints. Pinned in `tools/constraints.txt`, wired in with `-c` at every site | major | S | — | [specs/SPEC-leg-pin-and-share-dependencies.md](specs/SPEC-leg-pin-and-share-dependencies.md) | DONE |
...
| LEG-11 | `.devcontainer/` gitignored: development environment not shared. Now tracked and pinned to the CI ESP-IDF tag | minor | XS | — | [specs/SPEC-leg-pin-and-share-dependencies.md](specs/SPEC-leg-pin-and-share-dependencies.md) | DONE |
| LEG-12 | No `.gitattributes` despite `ci/factory.ps1` and Windows contributors. Root file added; the index held no CRLF, so nothing renormalised | minor | XS | — | [specs/SPEC-leg-pin-and-share-dependencies.md](specs/SPEC-leg-pin-and-share-dependencies.md) | DONE |
| LEG-13 | `duneos_logo.png` 1.5 MB tracked and referenced by nothing — the splash chain reads `boards/<board>/etc/splash/logo.png`. Deleted; a test now caps any tracked file at 256 KiB | minor | XS | — | [specs/SPEC-leg-pin-and-share-dependencies.md](specs/SPEC-leg-pin-and-share-dependencies.md) | DONE |
...
| ID | Finding | Severity | Size | Depends on | Spec | Status |
|---|---|---|---|---|---|---|
| LEG-17 | ADR 012 prescribed Greatest, a `dbt test` verb and `docs/testing.md`; the tree grew `tassert.h` and **7 suites** instead. ADR 041 supersedes it clause by clause, `dbt test` ships in `tools/dbt/testing.py`, `docs/testing.md` is written. LEG-39 closes with it: `pytest` is in `_DEPS`, so the venv dbt bootstraps can run the gate | major | M | — | [specs/SPEC-leg-17-18-19-21-tests-that-cover.md](specs/SPEC-leg-17-18-19-21-tests-that-cover.md) | DONE |
| LEG-18 | `test_known_yaml.c` compiled a copy of the wifi_daemon parser that had already diverged on the missing-file return the seed fallback branches on. One implementation now, `sdk/net/known_yaml.c`, compiled by the host suite; `iw` and `apps/user/wifi` link it too, which is how the empty-`ssid` bug surfaced — `iw` kept the entry and attached the following `psk:` to it, shifting every later one. **Validated on-board** on an m5stack-cardputer, 2026-09-13: the pre-existing `known.yaml` survived the sysbin flash and read back identical, `iw join` wrote a second entry with each `psk` on its own `ssid` and no shift, and `wifi_daemon` reread the file and reported `connected with SFR-ef60` (WPA2-PSK, channel 6) | major | S | — | [specs/SPEC-leg-17-18-19-21-tests-that-cover.md](specs/SPEC-leg-17-18-19-21-tests-that-cover.md) | DONE |
| LEG-19 | Untested pure logic. `tests/host/test_ld2450.c` covers both protocol worked examples, absent slots, bad header/tail and resync; `tools/dbt/tests/test_pure_logic.py` covers `validate_manifest`, `decode_perms` and `resolve` with their rejection paths | major | M | LEG-08 | [specs/SPEC-leg-17-18-19-21-tests-that-cover.md](specs/SPEC-leg-17-18-19-21-tests-that-cover.md) | DONE |
...
| LEG-40 | `dbt tui` exposes 13 actions and **no test action** (zero occurrences of qemu/pytest/test/fuzz in `tui.py`), though `_stream()` + `RichLog` already stream a `Popen` live. Blocked twice over: the TUI only shells to `dbt`, so it needs LEG-17's `dbt test`; and the QEMU gate is **unreachable without clobbering `.duneos_board`** — `dbt qemu` reads that file and refuses when `--board` differs (`qemu.py:925`), so exposing the action as-is ships a trap that breaks the developer's physical-board selection | minor | M | LEG-17 | [specs/SPEC-leg-36-40-dbt-verbs.md](specs/SPEC-leg-36-40-dbt-verbs.md) | **DONE** — `t` runs `dbt test`, `T` adds `--fuzz --qemu`, `x` cancels the child. ~~Second blocker~~: `testing._probe_qemu()` already reports the gate unavailable when `.duneos_board` names no QEMU board and never writes that file, so the TUI passes no `--board`. Exit 3 renders as "could not run", not as a failure |
...
LEG-36 and LEG-40 shipped together (SPEC-leg-36-40), so LEG-35 is what is left here.
