...
| LEG-39 | A fresh clone cannot run what CI runs: **2 of the 4 gates are CI-only** on the maintainer's own machine — `pytest` and a libFuzzer-capable `clang` are both absent, and ESP-IDF's bundled `esp-clang` cannot substitute (Xtensa/RISC-V backends only, no host `libclang_rt.fuzzer`, broken `ld.lld`). The CI-only fuzz gate is the one that FOUND LEG-34, so the next defect of that family is unreproducible locally. No `dbt test` verb exists either. **Overlaps LEG-17**, which already prescribes `dbt test` via ADR 012 — resolve the ownership before building | major | M | — | [specs/SPEC-leg-39-dbt-doctor-and-test.md](specs/SPEC-leg-39-dbt-doctor-and-test.md) | PROPOSED |
| LEG-40 | `dbt tui` exposes 13 actions and **no test action** (zero occurrences of qemu/pytest/test/fuzz in `tui.py`), though `_stream()` + `RichLog` already stream a `Popen` live. Blocked twice over: the TUI only shells to `dbt`, so it needs LEG-39's `dbt test`; and the QEMU gate is **unreachable without clobbering `.duneos_board`** — `dbt qemu` reads that file and refuses when `--board` differs (`qemu.py:925`), so exposing the action as-is ships a trap that breaks the developer's physical-board selection | minor | M | LEG-39 | [specs/SPEC-leg-40-tui-tests-menu.md](specs/SPEC-leg-40-tui-tests-menu.md) | PROPOSED |
