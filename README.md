...
DuneOS is past the POC stage. The kernel boots from on-chip flash even without an SD card, loads apps from `/flash/bin/`, `/sd/bin/`, or `/sd/apps/`, and exposes a stable typed ABI (v3) to userspace via `libdune.a`. Phase 24 (debt purge + hardening) is at **10/11 items shipped**, with Phase 24.9 (driver self-registration) the last open item — to close before starting Phase 25 (`dbt system` image recipes). The full phase status lives in [`ROADMAP_v2.md`](ROADMAP_v2.md); a quick snapshot:
...
| 24.9 — Kernel driver self-registration (ELF section, no more vfs_dev.c hardcoding) | 🟡 to-revisit before Phase 25 — 1st attempt blocked by ESP-IDF v6 linker constraints (see roadmap) |
