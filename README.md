...
| 24.7 — Safe boot & recovery (circuit breaker, USB MSC ordering, `--safe` flash) | 🟡 partial (4/5; hold-key TBD) |
...
If you flash an `init.yaml` whose services crash at boot, the supervisor will retry them per the restart policy. Two safety nets keep you out of a brick:
...
2. **Safe-mode flash (manual, host-side)** — if the breaker doesn't help (for example, every service trips and you want to start over):

   ```bash
   python tools/dbt.py flash sysbin --safe
   ```

   This re-flashes the sysbin partition with a minimal `init.yaml` containing only `usb_shell.dap` and `restart: always`. The board comes back to a usable shell, you mount it over USB MSC, fix `/flash/init.yaml`, then re-flash normally.
