...
DuneOS is past the POC stage. The kernel boots from on-chip flash even without an SD card, loads apps from `/bin`, `/sd/bin`, or `/sd/apps`, and exposes a stable typed ABI (v3) to userspace via `libdune.a`. Phase 24 (debt purge + hardening) is **11/11 shipped**, including 24.9 driver self-registration via GCC constructors. Next phase is 25 (`dbt system` image recipes). The full phase status lives in [`ROADMAP.md`](ROADMAP.md); a quick snapshot:
...
| 24.5 — Design Decisions / ADR (`docs/adr/`) | ✅ (42 ADRs) |
| 24.7 — Safe boot & recovery (circuit breaker, USB MSC ordering, recovery image profiles, hold-key recovery) | ✅ shipped |
...
echo /dev/ttyACM0     > .duneos_port      # used by dbt system flash
...
The image is composed from a *profile recipe*: a YAML listing the apps that go
into flash and the services that start at boot.
...
python tools/dbt.py system flash          # build + flash the image (profile-driven)
python tools/dbt.py system deploy /sd     # copy profile.apps_sd to SD
```

`system flash` packs the profile's apps plus the rendered `/init.yaml` into a
LittleFS image and writes it to the `sysbin` partition, which the kernel mounts
at the root at boot.
...
`init.yaml` lists the boot services. The kernel reads `/init.yaml` first, then `/sd/init.yaml` (the flash list comes from the active profile's `init_flash`, rendered by `dbt system flash`).

```yaml
services:
  - path: /bin/usb_shell.dap
    restart: always
  - path: /bin/wifi_daemon.dap
    restart: on-failure
```
...
2. **Hold-key safe boot (manual, device-side)** — if the board declares a `recovery_pin:` in its `board.yaml`, holding that button at power-on makes the kernel load `/init.yaml.safe` (which contains only `usb_shell.dap`) instead of the normal `/init.yaml` + `/sd/init.yaml` chain. Nothing is overwritten — the next normal boot reads `/init.yaml` again.
   ...
   `dbt system flash` always stages `/init.yaml.safe` next to the regular `init.yaml`, so this works as soon as the board has a usable recovery pin. Boards whose only free button doubles as the ESP32 boot strap pin (e.g. M5Stack CardPuter — GPIO0) have no `recovery_pin:` declared and fall through to nets #1 and #3.
...
3. **Safe-mode flash (manual, host-side)** — if neither net helps (for example, you want to permanently lock the board in safe mode until the next normal reflash):
   ...
   ```bash
   python tools/dbt.py system flash --profile cardputer-recovery
   ```

   A recovery profile ships `usb_shell` alone, so it becomes the *primary* `/init.yaml`. The board comes back to a usable shell, you mount it over USB MSC, fix `/init.yaml`, then re-flash normally.
...
├── tools/
│   ├── dbt/                   DuneBuild Tool (cli, builder, deploy, kernel, system, flashimg, tui, toolchain/)
...
- [ROADMAP.md](ROADMAP.md) — phase-by-phase plan, the legacy-audit debt section, and the current TODO list
