...
| 24.5 — Design Decisions / ADR (`docs/adr/`) | ✅ (41 ADRs) |
...
`init.yaml` lists the boot services. The kernel reads `/init.yaml` first, then `/sd/init.yaml` (per-board flash list is hand-edited in `boards/<board>/init.yaml` and copied as-is by `dbt flashimg`).
...
2. **Hold-key safe boot (manual, device-side)** — if the board declares a `recovery_pin:` in its `board.yaml`, holding that button at power-on makes the kernel load `/init.yaml.safe` (which contains only `usb_shell.dap`) instead of the normal `/init.yaml` + `/sd/init.yaml` chain. Nothing is overwritten — the next normal boot reads `/init.yaml` again.
   ...
   `dbt flashimg` always stages `/init.yaml.safe` next to the regular `init.yaml`, so this works as soon as the board has a usable recovery pin. Boards whose only free button doubles as the ESP32 boot strap pin (e.g. M5Stack CardPuter — GPIO0) have no `recovery_pin:` declared and fall through to nets #1 and #3.
...
3. **Safe-mode flash (manual, host-side)** — if neither net helps (for example, you want to permanently lock the board in safe mode until the next normal reflash):
   ...
   This re-flashes the sysbin partition with the safe-mode init.yaml as the *primary* `/init.yaml`. The board comes back to a usable shell, you mount it over USB MSC, fix `/init.yaml`, then re-flash normally.
