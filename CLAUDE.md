...
| Phase 24.5 — Design Decisions (ADR) | **DONE** | `docs/adr/` — 11 short ADRs (1 page each, ~50 lines): process, error model (`int`/-errno), OSAL API (static-storage handles only), memory caps (`OSAL_MEM_*` with silent DRAM fallback), task priorities (5 symbolic levels), path conventions (`/flash` mandatory), manifest extensibility (unknown fields ignored), multi-arch smoke test (CI before Phase 29), memory fragmentation (4 pillars + no compaction), **driver kernel/userspace boundary** (4-criterion test + matrix; identifies 3 existing drifts: BQ27220/ST7789/GPIO expanders), **architecture-specific accelerators** (3 patterns: capability HAL, arch-only SDK lib, partial-overlap HAL — drives the `hal_encoder` PCNT/PIO refactor). All consumed by Phases 24 debt + 25-29. |
