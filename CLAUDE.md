...
- **`/dev/spi-N` shares one `spi_device_handle_t` per CS pin across fds** — naïvely calling `spi_bus_add_device(cs=37)` from two concurrent apps causes ESP-IDF to warn `"GPIO N is conflict with others and be overwritten"` and corrupts subsequent transactions (symptom observed 2026-05-20: `gfx_demo` exited 1 when `g_shell.dap` was also using the display). Fixed in `drv_spi.c` 2026-05-20 by a per-bus device pool keyed by CS pin: the first fd to call `ioctl(SPI_SET_CS, N)` creates the ESP-IDF device handle, subsequent fds with the same CS share it via refcount. Last writer wins for `SPI_SET_MODE` / `SPI_SET_SPEED` reconfigs — acceptable when the fds talk to the same chip (the natural sharing case). Pool capacity = `MAX_DEVICES_PER_BUS` (8) distinct CS pins per bus; bumping it is a one-line constant change.
