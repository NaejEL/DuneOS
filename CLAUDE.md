...
| Phase 24.5 — Design Decisions (ADR) | **DONE** | `docs/adr/` — 17 short ADRs (1 page each): process, error model, OSAL API, memory caps, task priorities, path conventions, manifest extensibility, multi-arch smoke test, memory fragmentation, driver kernel/userspace boundary, architecture accelerators (Pattern A), threat model, test strategy, network architecture (POSIX sockets + vendored lwIP + per-medium HAL), capability-based source resolution, declarative-driven kernel/userspace, **captured-app exit semantics** (setjmp/longjmp pattern — fixes the shell-killing footgun observed 2026-05-20). |
| Phase 24.7 — Safe boot & recovery | Not finished | Items shipped (circuit breaker, USB MSC ordering doc, `dbt flash sysbin --safe`, README recovery section) sit in commits but the phase is NOT closed — `board.yaml recovery_pin` + hold-key implementation in `init.c` still required to ship the full scope. Don't claim DONE until that lands and is hardware-tested. |
| Phase 24.8 — `<duneos/board.h>` auto-generated by dbt (ADR 015 P2) | Not finished | `boardgen.py` works for storage/display/input/i2c sections and `libdisp.c` consumes `DUNEOS_DISPLAY_DEV`. NOT closed until: capabilities-based filtering of emitted defines, `board.yaml` mtime rebuild trigger in dbt, `libst7789.c` migrated (depends on Phase 24 debt #6+#2). |
...
| Phase 24.9.5 — Captured-app exit semantics (ADR 016) | Not started | `loader_run_captured` installs setjmp; `duneos_exit` longjmps back instead of `vTaskDelete(NULL)` (which currently kills the shell when a captured app exits). Fixes the gfx_demo footgun observed 2026-05-20. App-author contract: no pthread_create from captured, free fds/heap before exit. ~50 LoC. **Critical for stability before the contest launcher.** |
| Phase 24.10 — libgfx streaming mode | Not started | Adds `gfx_open(GFX_MODE_STREAM)` that draws line-by-line to `/dev/disp0` without a userspace back-buffer. Saves 64 KiB per gfx app on 240x135. Required so the contest launcher + 1-2 gfx apps can coexist on CardPuter's 320 KiB DRAM. `gfx_pixel` is BUFFERED-only; STREAM exposes `gfx_fill_rect`/`gfx_text`/`gfx_hline`/`gfx_vline`. ~100 LoC. |
...
- **`while (running_count > N) usleep(...)` busy-polls and may hang** — the shell at app priority 2 shared the CPU with the spawned app at the same priority. The supervisor's exit processing could be starved long enough that the shell never observed the count drop, or the exit event was processed *between* poll iterations and the shell missed nothing but the loop kept running because the supervisor hadn't yet decremented. Replaced with `duneos_supervisor_wait_for_completion(target_count)` which blocks on a counting semaphore signalled exactly once per processed exit. No busy-wait, no race window. Apply this pattern any time the shell or another task needs to wait for app completion.
...
- **`/dev/spi-N` cannot be shared by two concurrent apps using the same chip** — each `open("/dev/spi-1")` adds a `spi_device_handle_t` to the ESP-IDF SPI bus with the CS GPIO that the app sets via `ioctl(SPI_SET_CS, ...)`. If two apps configure the same CS pin (e.g. `g_shell` declared as a daemon in `init.yaml` AND `gfx_demo` launched from the CDC shell — both want CS=37 for the display), `spi_bus_add_device` warns `"GPIO N is conflict with others and be overwritten"` and from there transactions go to the wrong device handle, eventually `backend_open` returns NULL → app exits non-zero. **Today's rule**: don't declare an interactive display-using app (g_shell, gfx_demo) as a daemon in `init.yaml`. Run them on demand from the CDC shell. Proper multi-owner SPI arbitration (a kernel-side display ownership token, or refcounted device handles per CS) is a future ADR — see backlog. Observed 2026-05-20 on CardPuter: `gfx_demo` exited 1 with the conflict warning when `g_shell.dap` was in `init.yaml`; removing it fixed the issue.
...
- **Architecture Decision Records** — [`docs/adr/`](docs/adr/) (17 ADRs as of 2026-05). Start at [`docs/adr/README.md`](docs/adr/README.md). Authoritative for design intent.
