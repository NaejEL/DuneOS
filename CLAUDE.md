...
- **A test must construct what it asserts against, and must never read a gitignored artefact** — LEG-38, four recorded instances, every one of them green. `tools/dbt/tests/test_bspgen_uart.py` (PR #5) and `test_sdkconfig_check.py` (PR #7) asserted against `boards/*/sdkconfig.board`, gitignored at `.gitignore:49`, and passed only where bspgen had run; a collection-time `ImportError` (`dbt.tui` → `textual`) left 202 tests unrun while the job read green; a submodule-less checkout killed `make` before the pytest step could start (`6395ac4`); and three assertions behind `if not generated.exists(): continue` had never once executed. The last form is the worst — nothing anywhere goes red. **It is now enforced rather than reviewed**: the root `conftest.py` fails any test that opens a path git ignores, on every machine, present or not. Regenerate what you assert against into `tmp_path` (the `regenerated_root` fixture), and let CI's count floor and self-proof step keep the job honest.
- **A test must not invoke the project's own CLI** — `tools/dbt.py` bootstraps `tools/.dbt-venv` and `pip install`s seven packages from PyPI whenever the interpreter is not already inside that venv, which is exactly CI's case. Three unit tests that shelled out to it therefore depended on the network and wrote a venv into the working tree; a PyPI hiccup would have reddened them for a reason unrelated to what they test. Enter through `dbt.cli.main()` with `PYTHONPATH=tools` instead. Measured: the suite went from 17.5 s to 3.8 s for the same 490 tests — the bootstrap was running on every CI run. Sibling of the lesson above: that one reads a build artefact, this one *creates* one.
...
- **cJSON is the `third_party/cjson` submodule, compiled in place by the loader** (`duneos_loader/CMakeLists.txt:7`) — ESP-IDF v6.0.1 no longer ships `json` as a built-in component. Never put `json` or `espressif__cjson` back into the loader's REQUIRES. It is not vendored under `duneos_loader/src/`, which holds only `elf_parse.c`, `loader.c` and `manifest_depth.c`; a clone without `--recursive` has no cJSON at all.
