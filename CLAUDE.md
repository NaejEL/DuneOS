...
- **`duneos_exit()` from a captured app kills the caller's task** — `vTaskDelete(NULL)` in a captured context kills the shell. Fix in Phase 24.9.5 via setjmp/longjmp. Until then: use `heap_size > 0` to force spawned mode.
