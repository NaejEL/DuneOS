...
| Phase 24.5 — Design Decisions (ADR) | **DONE** | `docs/adr/` — 9 short ADRs (1 page each, ~50 lines): process, error model (`int`/-errno), OSAL API (static-storage handles only), memory caps (`OSAL_MEM_DEFAULT`/`EXTERNAL`/`DMA`/`FAST` with silent DRAM fallback + `osal_mem_caps_available()`), task priorities (5 symbolic levels `IDLE`/`LOW`/`NORMAL`/`HIGH`/`RT`), path conventions (`/flash` mandatory + `/proc`/`/sys` reserved), manifest extensibility (unknown fields ignored, no ABI bump on additive changes), multi-arch smoke test (CI on ESP32-S3 + ESP32-C6 + Linux sim before Phase 29), memory fragmentation strategy (4 pillars: static-first, per-app pool, TLSF, caps placement; no compaction without MMU). All consumed by Phases 25-29. |
