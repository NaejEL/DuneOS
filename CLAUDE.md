...
| Phase 24.7 — Safe boot & recovery | **PARTIAL** | 4 of 5 items shipped 2026-05-20: circuit breaker (3 crashes / 30 s default → restart disabled, klog warning), USB MSC ordering verified+documented, `dbt flash sysbin --safe` (rewrites init.yaml to usb_shell-only at staging time), README recovery section. Remaining: hold-key-at-boot fallback via `board.yaml` `recovery_pin` — defers to a session with a board on hand for hardware testing. |
| Phase 24.8 — `<duneos/board.h>` auto-generated by dbt (ADR 015 P2) | **MVP** | `tools/dbt/boardgen.py` writes `<app_build_dir>/_board.h` from `board.yaml` (4 sections: storage, display, input, i2c). `builder.py` adds `-I<build_dir>`. Public alias `kernel/duneos_kernel/include/duneos/board.h` does `#include "_board.h"`. libdisp.c migrated to `DUNEOS_DISPLAY_DEV`. Reserved for follow-up: capability-based filtering, board.yaml mtime rebuild trigger, libst7789 migration (depends on Phase 24.6). |
