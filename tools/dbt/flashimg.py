...
def _stage(staging_dir: Path) -> int:
    ...
    a minimal /flash/init.yaml for no-SD boot.  Returns the number of staged apps.
    ...
    # Known service apps that should auto-start at boot (restart: always).
    # These are looked up by manifest name, so renaming an app updates this too.
    _BOOT_SERVICES = ["usb_shell", "shell", "wifi_daemon"]
    services_yaml = ""
    for svc in _BOOT_SERVICES:
        if svc in staged:
            services_yaml += f"  - path: /flash/bin/{svc}.dap\n    restart: always\n"

    init_yaml_content = (
        "# DuneOS /flash/init.yaml — generated by dbt flashimg\n"
        "# Services here start even without an SD card (e.g. when USB MSC is active).\n"
        "# Add board-specific services to /sd/init.yaml instead.\n"
        "services:\n"
        + (services_yaml if services_yaml else "  # (no boot services found — add manually)\n")
    )
