...
def cmd_system_deploy(args) -> None:
    ...
    for app_name in targets:
        ...
        if not deploy_single(app_dir, sd_path, is_bin):
            ...
            n_ok += 1
    # Write /sd/init.yaml from profile.init_sd if non-empty. Same shape as
    # the flash init.yaml; otherwise the SD-side `after:` deps configured in
    # the TUI editor are dropped on the floor.
