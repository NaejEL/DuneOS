...
- **Multi-owner `/dev/spi-N` arbitration** — today each `open` adds a separate ESP-IDF `spi_device_handle_t` with the app's CS pin. Two apps with the same CS conflict; the warning `"GPIO N is conflict with others and be overwritten"` is followed by corrupted transactions. Workaround documented in CLAUDE.md (don't run two display-using apps concurrently). Real fix needs either (a) refcounted shared device handles per CS, or (b) a kernel-side display ownership token. ADR needed before implementing — design space includes "first opener wins, others get EBUSY", "second opener kicks first" (à la TTY HUP), or "explicit `disp_acquire`/`release`". Wait until a real concurrent use case beyond the gfx_demo+g_shell incident appears.
