...
| [026](026-file-type-associations.md) | File-type associations — apps declare what they open, not a central table | Proposed 2026-06-07 | "Open with": each app's manifest declares `opens: [ext, …]` (freedesktop `MimeType=` / Android intent-filter shape); a resolver scans installed apps and launches the handler, the explorer's built-in viewers being the fallback. Decentralised like [023](023-app-icon-assets.md)/[015](015-declarative-driven-architecture.md). Needs a launch-arg/IPC handoff to pass the path (sub-decision). |
